...
def generate_screenshots(video_path, output_folder, num_screenshots=10):
    ...
    for _ in range(num_screenshots):
        ...
        subprocess.run(
            ["ffmpeg", "-ss", str(random_time), "-i", str(video_path),
             "-frames:v", "1", "-q:v", "2", "-y", "-loglevel", "error", str(screenshot_path)],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_folder", type=str, help="Folder to save the screenshots")
    args = parser.parse_args()

    process_videos_in_folder(args.input_folder, args.output_folder or Path(args.input_folder) / "metadata")
