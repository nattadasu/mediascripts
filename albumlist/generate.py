...
def get_metadata(file_path: str) -> Union[Dict[str, Union[str, int]], None]:
    if file_path.endswith('.mp3'):
        ...
        return None
    return {
        'title': audio.get(title_key, [''])[0],
        'titlesort': audio.get('titlesort', [''])[0] if 'titlesort' in audio else None,
        'artist': audio.get(artist_key, [''])[0],
        'duration': int(audio.info.length),
    }

def print_metadata(index: Union[str, int], metadata: Dict[str, Union[str, int]]):
